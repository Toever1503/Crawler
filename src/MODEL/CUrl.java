			...
			case 53: // --data-urlencode
				...
				case -1: // no '='
					if ((atIdx = data.indexOf("@")) >= 0) { // [name]@filename
						...
						try {
							data = prefix + URLEncoder
									.encode(Util.b2s(readInput(getIO(data.substring(atIdx + 1))), null, ""), charset);
						} catch (Exception e) {
							lastEx = e;
						}
						break;
...
//				break;
		...
		if (dataSb.length() > 0 && form.size() > 0 || dataSb.length() > 0 && "HEAD".equals(method)
...
//						if (!ignoreContentLength) {
	...
	@SuppressWarnings({ "WeakerAccess", "JavaDoc", "ConstantConditions", "ResultOfMethodCallIgnored",
			"StatementWithEmptyBody", "UnusedReturnValue", "SuspiciousMethodCalls" })
	final static class Util {
		...
		public static Map<String, String> split(String s, String entryDelim, String kvDelim,
